            ...
            */
            function(workers, rounds, addressAccount, callback) {
                ...
                var trySend = function (withholdPercent) {
                    ...
                    var addressAmountsNew = {};
                    var needPayCount = Object.keys(addressAmounts).length;
                    var hasPayCount = 0;
                    for (var a in addressAmounts) {
                        //addressAmounts[a] = coinsRound(addressAmounts[a]);
                        daemon.cmd('validateaddress', [a], function (result) {
                            if (result.error) {
                                return;
                            } else {
                                if (result[0].response.isvalid) {
                                    var a = result[0].response.address;
                                    addressAmountsNew[a] = coinsRound(addressAmounts[a]);
                                } else {
                                    logger.error(logSystem, logComponent, 'Error address format ' + a);
                                }

                                hasPayCount += 1;
                                if (hasPayCount === needPayCount) {
                                    addressAmounts = addressAmountsNew;
